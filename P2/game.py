...
#funciones de los print necesarios
	...
	def print_dead_characters(self):
		...
		for c in self.dead_characters:
			mg = (mg+str(num)+".- "+c.__class__.__name__+"\n")
			num = num + 1
		return mg
	...
	def print_damage(self, num, enemy, damage):
		return(str(type(self.characters[num]).__name__)+" did "+str(damage)+" damage to "+str(type(self.enemies[enemy]).__name__)+"."+ str(type(self.enemies[enemy]).__name__)+ " has "+ str(self.enemies[enemy].hp)+" hp left.")
...
#funciones para añadir jugadores y enemigos además de controlar si mueren o no
	def add_player(self, eleccion, c_socket):
		if (eleccion == 1):
	...
	def character_dead(self, random_charac):
		...
		if (self.characters[random_charac].hp_real <= 0):
			...
			dead = str(name)+" has been defeated. It can not make any move until revived.\n"
...
#funciones para elegir el ataque
	def eleccion_attack(self, num, jugador, charac):
...
#funciones para controlar el turno de los jugadores y enemigos
	def players_turn(self, charac):
		self.print_players_turn()
		#while ((num < len(self.characters))and (len(self.enemies) > 0) and not finish_turn):
		damage, cura, charac = self.eleccion_attack(self.current_turn, self.current_turn + 1, charac)
		if (charac == 's' and (type(self.characters[self.current_turn]).__name__ == "Procrastinator")):
			dam = self.hit_all_enemies(self.current_turn, damage)
		elif (damage > 0):
			dam = self.hit_an_enemy(self.current_turn, damage)
		elif (damage == 0 and cura == 0):
			dam = (type(self.characters[len(self.characters)-1]).__name__+" has returned to the game")
		else:
	...
	def enemies_turn(self):
		...
		while (num < len(self.enemies) and (len(self.characters) > 0)):
			...
			dam = str(type(self.enemies[num]).__name__)+" did "+ str(damage)+" damage to "+ str(type(self.characters[random_charac]).__name__)+". "+ str(type(self.characters[random_charac]).__name__)+" has "+ str(self.characters[random_charac].hp_real)+" hp left. \n"
			num = num + 1
